...
class ExperimentVOT(object):
    ...
    """
    def __init__(self, root_dir, version=2017, read_image=True, experiments=('supervised'),
    ...
    def run_supervised(self, tracker, visualize=False):
        ...
        for s, (img_files, anno, _) in tqdm(enumerate(self.dataset)):
    ...
    def run_realtime(self, tracker, visualize=False):
        ...
        for s, (img_files, anno, _) in enumerate(self.dataset):
            ...
            # tracking loop
            for f, img_file in enumerate(img_files):
                image = Image.open(img_file)# PIL 读取的图像没有 .shape属性
                if self.read_image:
                    #frame = image 
                    frame= cv2.imread(img_file, cv2.IMREAD_COLOR)
                else:
    ...
    def report(self, tracker_names):
        ...
        for name in tracker_names:
            ...
            for s, (img_files, anno, meta) in enumerate(self.dataset):
               ...
               # ious[seq_name][:len(seq_ious), :] = seq_ious #这里不能直接复制，否则会报错
                ...
                ious_full[seq_name]=seq_ious_full
                ...
                failures[seq_name]=seq_failures
        ...
        return accuracy,robustness,speed
