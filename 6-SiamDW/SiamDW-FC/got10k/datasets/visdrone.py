...
class VisDrone(object):
    ...
    def __init__(self, root_dir):
        ...
        self.anno_files = sorted(glob.glob(
            os.path.join(root_dir, 'annotations/*_s.txt')))

        self.seq_dirs = sorted(glob.glob(
            os.path.join(root_dir, 'sequences/*')))

        # self.seq_dirs = [os.path.dirname(f) for f in self.img_files]
    ...
    def __getitem__(self, index):
        ...
        img_files = sorted(glob.glob(os.path.join(self.seq_dirs[index], '*.jpg'))) #UAVDT
