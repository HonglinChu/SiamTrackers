...
from trtr.utils.bbox import get_axis_aligned_bbox
...
def parse_range_int(range_str):
    ...
    #return np.arange(*param)
    return np.array(param)
# pk-0.150_wi-0.476_lr-0.380
parser = argparse.ArgumentParser(description='Hyperparamter Search')
parser.add_argument('--snapshot', default='./models/pretrained/checkpoint_e4.pth',type=str, help='snapshot of model')
parser.add_argument('--dataset', default='VOT2019',type=str, help='dataset name to eval')
parser.add_argument('--penalty-k', default='0.1480, 0.1485, 0.1490, 0.1495, 0.1500, 0.1505, 0.1510, 0.1515, 0.1520', type=parse_range) # 0.16  
parser.add_argument('--lr', default='0.3780, 0.3785, 0.3790, 0.3795', type=parse_range) # 0.30   
parser.add_argument('--window-influence', default='0.4755, 0.4760, 0.4765, 0.4770, 0.4775, 0.4780, 0.4785', type=parse_range) #0.40  
parser.add_argument('--search-region', default='255', type=parse_range_int) # 287 
parser.add_argument('--config', default='./models/trtr/config.yaml', type=str) 
...
if __name__ == '__main__':
    ...
    benchmark_path = os.path.join('hp_search_result', model_name, args.dataset)
