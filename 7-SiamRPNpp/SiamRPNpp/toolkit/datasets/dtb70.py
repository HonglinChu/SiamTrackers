...
class DTB70Video(Video):
    ...
    def __init__(self, name, root, video_dir, init_rect, img_names,
        ...
        super(DTB70Video, self).__init__(name, root, video_dir,
...
class DTB70Dataset(Dataset):
    ...
    def __init__(self, name, dataset_root, load_img=False):
        ...
        self.anno_files = sorted(glob.glob(
            os.path.join(dataset_root, '*/groundtruth_rect.txt')))
        ...
        for  i in range(len(self.seq_names)):
            ...
            img_names=[x.split('/DTB70/')[-1] for x in img_names]
        ...
        for  video in pbar:
           #pbar.set_postfix_str(video)
            self.videos[video] = DTB70Video(video,
                                          dataset_root,
