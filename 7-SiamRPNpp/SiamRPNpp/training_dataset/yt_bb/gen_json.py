...
instanc_size = 511
abspath='/media/ubuntu/942dfa3a-9dbd-486b-8be7-a3f77dcbf943/Datasets/YOUTUBE/'
crop_path = abspath+'crop{:d}'.format(instanc_size)
...
def parse_and_sched(dl_dir=''):
    ...
    for d_set in d_sets:
       ...
       # df = pd.DataFrame.from_csv(d_set+'.csv', header=None, index_col=False)
        df=pd.read_csv(abspath+d_set+'.csv', header=None, index_col=False)
        ...
        if 'yt_bb_detection_train' == d_set:
            json.dump(js, open(abspath+'train.json', 'w'), indent=4, sort_keys=True)
        else:
            json.dump(js, open(abspath+'val.json', 'w'), indent=4, sort_keys=True)
        js = {}
