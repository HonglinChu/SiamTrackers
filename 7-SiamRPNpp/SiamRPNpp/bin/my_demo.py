...
def select_model(arg):
    if arg==1:
        config='experiments/siamrpn_alex_dwxcorr/config.yaml'         #SiamRPN   -AlexNet       180fps
        snapshot='experiments/siamrpn_alex_dwxcorr/model.pth'

    elif arg==2:
        config='experiments/siamrpn_alex_dwxcorr_otb/config.yaml'     #SiamRPN   -AlexNet -OTB  180fps
        snapshot='experiments/siamrpn_alex_dwxcorr_otb/model.pth'
    
    elif arg==3: 
        config='experiments/siamrpn_r50_l234_dwxcorr/config.yaml'     #SiamRPN++ -ResNet        35fps  
        snapshot='experiments/siamrpn_r50_l234_dwxcorr/model.pth'
    
    elif arg==4:
        config='experiments/siamrpn_r50_l234_dwxcorr_otb/config.yaml' #SiamRPN++ -ResNet  -OTB  35fps
        snapshot='experiments/siamrpn_r50_l234_dwxcorr_otb/model.pth'
    
    elif arg==5:
        config='experiments/siamrpn_r50_l234_dwxcorr_lt/config.yaml'  #SiamRPN++ -ResNet  -LT   20fps
        snapshot='experiments/siamrpn_r50_l234_dwxcorr_lt/model.pth'
    
    elif arg==6:
        config='experiments/siamrpn_mobilev2_l234_dwxcorr/config.yaml'#SiamRPN++ -Mobilev2      75fps
        snapshot='experiments/siamrpn_mobilev2_l234_dwxcorr/model.pth'
    
    elif arg==7:
        config='experiments/siammask_r50_l3/config.yaml'              #SiamMask  -ResNet        56fps
    ...
    video='testing_dataset/rssrai/1_Car_02/img/' #修改成你自己的测试视频路径（可以选择OTB视频测试）

    return config, snapshot, video, param

config,snapshot,video,param=select_model(1)
