...
snapshot='models/pretrained_models/checkpoint_e41-16-up.pth'

parser = argparse.ArgumentParser(description='siamrpn tracking')
parser.add_argument('--dataset', '-d', default='VOT2016',type=str,help='datasets')
parser.add_argument('--tracker_name', '-t', default='SiamRPNpp-UP',type=str,help='tracker  name')
     ...
     #   for v_idx, video in enumerate(dataset):
        ...
        for  video in tqdm(enumerate(dataset)):
            ...
            if 'VOT2018-LT' == args.dataset:
                ...
                video_path = os.path.join('results', args.dataset, args.tracker_name, video[1].name)
                if not os.path.isdir(video_path):
                    os.makedirs(video_path)
                result_path = os.path.join(video_path, '{}_001.txt'.format(video[1].name))
                with open(result_path, 'w') as f:
                    for x in pred_bboxes:
                        f.write(','.join([str(i) for i in x])+'\n')
                result_path = os.path.join(video_path,
                ...
                result_path = os.path.join(model_path, '{}.txt'.format(video[1].name))
                with open(result_path, 'w') as f:
                    for x in pred_bboxes:
                        f.write(','.join([str(i) for i in x])+'\n')
