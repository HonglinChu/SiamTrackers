...
parser.add_argument('--config', default='models/siammask_resnet/config-base.yaml' , type=str,help='config file')
parser.add_argument('--snapshot', default='models/siammask_resnet/snapshot/checkpoint_e26.pth', type=str,help='snapshot of models to eval')
     ...
     #   for v_idx, video in enumerate(dataset):
        ...
        for  video in tqdm(enumerate(dataset)):
            ...
            if 'VOT2018-LT' == args.dataset:
                ...
                video_path = os.path.join('results', args.dataset, args.tracker_name, video.name)
                if not os.path.isdir(video_path):
                    os.makedirs(video_path)
                result_path = os.path.join(video_path, '{}_001.txt'.format(video.name))
                with open(result_path, 'w') as f:
