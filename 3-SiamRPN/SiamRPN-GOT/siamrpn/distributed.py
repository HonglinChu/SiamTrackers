...
from siamrpn.log_helper import log_once
...
def _dist_init():
    ...
    if guess right:
        ...
        proc_id: rank
    '''
    rank = int(os.environ['RANK']) #获取GPU的全局的rank
    num_gpus = torch.cuda.device_count()    # 获取node的GPU的数量, 
    torch.cuda.set_device(rank % num_gpus)  # 设置当前运行脚本使用的GPU, 或者说是local rank 
    dist.init_process_group(backend='nccl') # 初始化进程通信方式
    world_size = dist.get_world_size()  # 获取全局rank 和 node的数量
...
def reduce_gradients(model, _type='sum'): # avg 
    types = ['sum', 'avg']
    ...
    if get_world_size() > 1:
        for param in model.parameters():
            if param.requires_grad:
                dist.all_reduce(param.grad.data) #对不同设备之间的数据进行求和操作
                if _type == 'avg':
